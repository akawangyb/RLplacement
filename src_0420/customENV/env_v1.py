...
class CustomEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # 定义请求信息和服务器信息
        self.user_request_info = get_user_request_info(self.end_timestamp, self.user_number)
        self.server_info = get_server_info(self.server_number)
        self.container_info = get_container_info(self.container_number)
    ...
    def step(self, action):
        ...
        # 传入的placing_action是一个二维0,1矩阵[server_number,container_number]
        # 传入的routing_action是一个一维[user_number],每个值表示这个用户请求由哪个服务器完成
    ...
    def cal_placing_rewards(self, placing_action):
        server_storage_demand = [0] * self.server_number

        for server_id, container_list in enumerate(placing_action):
            sum_storage = 0
            for container_id, is_placed in enumerate(container_list):
                sum_storage += is_placed * self.container_info[container_id][0]  # 'container_size'
            server_storage_demand[server_id] = sum_storage

        # 检查是否超过
        is_valid_placing_action = [True] * self.server_number
        for server_id in range(self.server_number):
            if server_storage_demand[server_id] > self.server_info[server_id][4]:  # storage_size
                is_valid_placing_action[server_id] = False
        ...
        for server_id in range(self.server_number):
            if not is_valid_placing_action[server_id]:
                ...
                # 如果是一个合法的placing
                # 计算上一时隙 当前server的placing_action
                # 只有当前是1，上一次是0才需要加上延迟
                now_placing_list = placing_action[server_id]
                last_placing_list = self.state['last_placing_state'][server_id]
                for container_id in range(self.container_number):
                    if now_placing_list[container_id] == 1 and last_placing_list[container_id] == 0:
                        placing_rewards[server_id] += self.container_info[container_id][1]  # 'container_pulling_delay'
