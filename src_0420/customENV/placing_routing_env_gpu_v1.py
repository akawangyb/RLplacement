...
# 描述: maddpg请求和路由的环境, 将每个智能体获得的奖励设置为全局奖励,将所有的数据放到gpu里面
# 作者: WangYuanbo
# --------------------------------------------------
...
class CustomEnv(gym.Env):
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, dict]:
        ...
        for server_id, server_action in enumerate(placing_action):
            # 获得具体的部署动作
            server_action = torch.argmax(server_action)
        ...
        for user, user_action in enumerate(routing_action):
            ...
            # 这里要检查一下
            server_index = torch.argmax(user_action)
        ...
        # 先找出哪个服务器超过了
        for user, user_action in enumerate(routing_action):
            server_id = torch.argmax(user_action)
        ...
        # 假设每个智能体的奖励是全局奖励
        final_rewards = -np.full(self.routing_agents_number + self.placing_agents_number,
                                 sum(placing_rewards) + sum(routing_rewards))
        # return return_state, -np.array(placing_rewards + routing_rewards), done, info
        info = -np.array(placing_rewards + routing_rewards)
        return return_state, final_rewards, done, info
