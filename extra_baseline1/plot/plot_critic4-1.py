...
path = r'../log_res/ablation2/4-1/critic/critic.loss'
path = [r'../log_res/ablation2/4-1/critic140/critic.loss',
        r'../log_res/ablation2/4-1/critic140']
...
draw_imitation_fig1(ax1, title[0], path[0],annotate_text=r'$\times10^{9}$', critic=True)

ax2 = fig.add_subplot(gs[2:, 0])
file_paths = find_name_log_folders(path[1], agent_name='imitation_learning')
draw_imitation_fig2(ax2, file_paths, annotate_text=r'$\times 10^5$', title=title[1], tag='lmbda')

index = 0
# 右边上面的第一个图
ax3 = fig.add_subplot(gs[0, 1])
path = r'../log_res/ablation2/4-1/critic140/imitation_learning_env-000-20250302-224417/output_info.log'
draw_sub_fig(ax3, title='$\mathrm{(c)}$ $\mathrm{Step=0}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边上面的第二个图
ax4 = fig.add_subplot(gs[1, 1])
path = r'../log_res/ablation2/4-1/critic140/imitation_learning_env-20250307-104333/output_info.log'
draw_sub_fig(ax4, title='$\mathrm{(d)}$ $\mathrm{Step=10}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边下面的第一个图
ax5 = fig.add_subplot(gs[2, 1])
path = r'../log_res/ablation2/4-1/critic140/imitation_learning_env-050-20250303-002218/output_info.log'
draw_sub_fig(ax5, title='$\mathrm{(e)}$ $\mathrm{Step=50}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边下面的第二个图
ax6 = fig.add_subplot(gs[3, 1])
path = r'../log_res/ablation2/4-1/critic140/imitation_learning_env-150-20250303-003511/output_info.log'
draw_sub_fig(ax6, title='$\mathrm{(f)}$ $\mathrm{Step=150}$的后续训练收益', file_path=path, i=index)
