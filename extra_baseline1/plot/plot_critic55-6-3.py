...
def draw_imitation_fig2(ax, file_paths, title='$(\mathrm{b})$不同模仿学习回合下的后续训练收益',
    ...
    for i, file_path in enumerate(file_paths):
        ...
        total_rewards = sliding_average(total_rewards, 2)
        # 绘制total reward的折线图
...
path = r'../log_res/ablation2/6-3/critic/critic_loss.log'
path = [r'../log_res/ablation2/6-3/critic77/critic_loss.log',
        r'../log_res/ablation2/6-3/critic77']
...
path = r'../log_res/ablation2/6-3/critic0/imitation_learning_env-0-20250301-210049/output_info.log'
draw_sub_fig(ax3, title='$\mathrm{(c)}$ $\mathrm{Step=0}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边上面的第二个图
ax4 = fig.add_subplot(gs[1, 1])
path = r'../log_res/ablation2/6-3/critic0/imitation_learning_env-024_20250304-161902/output_info.log'
draw_sub_fig(ax4, title='$\mathrm{(d)}$ $\mathrm{Step=24}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边下面的第一个图
ax5 = fig.add_subplot(gs[2, 1])
path = r'../log_res/ablation2/6-3/critic0/imitation_learning_env-050-20250304-164137/output_info.log'
draw_sub_fig(ax5, title='$\mathrm{(e)}$ $\mathrm{Step=50}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边下面的第二个图
ax6 = fig.add_subplot(gs[3, 1])
path = r'../log_res/ablation2/6-3/critic0/imitation_learning_env-100-20250304-170427/output_info.log'
