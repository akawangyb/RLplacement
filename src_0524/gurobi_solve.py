...
def model_assisted_random_solve(env: CustomEnv, timestamp: int, trade_factor=0.5):
    """
    优化目标添加一个随机因子
    :param env:
    :param timestamp:
    ...
    # objective = quicksum(y[u, n] for u in range(env.user_number) for n in range(env.server_number))
    # m.setObjective(objective, GRB.MAXIMIZE)
    edge_routing = quicksum(y[u, n] for u in range(env.user_number) for n in range(env.server_number))
    cloud_routing = quicksum(y[u, env.server_number] for u in range(env.user_number))
    objective = edge_routing * trade_factor + (1 - trade_factor) * cloud_routing
    m.setObjective(objective, GRB.MAXIMIZE)
...
if __name__ == '__main__':
    ...
    timestamp = 0
    while not done:
        x_result, y_result = model_assisted_random_solve(env, timestamp)
    ...
    timestamp = 0
    while not done:
        x_result, y_result = model_assisted_random_solve(env, timestamp, trade_factor=1)
        placing_action = torch.tensor(x_result).int()
        routing_action = torch.tensor(y_result).int()
        routing_action = torch.argmax(routing_action, dim=1)
        env_action = {
