...
class CustomEnv(gym.Env):
    ...
    def step(self, action):
        ...
        valid_action = torch.all(is_valid_placing_action) & torch.all(is_valid_routing_action)

        # 如果invalid_action中有任何元素是True，就把 reward 设置为 1
        if not valid_action:
            rewards = torch.Tensor([self.penalty]).to(self.device)
        else:
            rewards = torch.Tensor([torch.sum(placing_rewards) + torch.sum(routing_rewards)]).to(self.device)
    ...
    def cal_routing_rewards(self, placing_action, is_valid_placing_action, routing_action):
        ...
        is_valid_routing_action = torch.zeros(self.user_number).bool().to(self.device)
        ...
        is_valid_routing_action[cloud_routing_mask] = True
        ...
        is_valid_routing_action[valid_edge_routing_mask] = True

        # 3. 路由到边，不正确
        invalid_edge_routing_mask = (
                ~ cloud_routing_mask & (invalid_storage_mask | not_deployed_mask | insufficient_resource_mask))
        routing_rewards[invalid_edge_routing_mask] = self.penalty
        is_valid_routing_action[invalid_edge_routing_mask] = False
