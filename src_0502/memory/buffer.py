...
class MultiAgentReplayBuffer:
    ...
    def __init__(self, state_size, actions_size, buffer_size, device='cpu'):
        # state, action, reward, next_state, done
        self.state = torch.empty(buffer_size, state_size, dtype=torch.float, device=device)
        self.actions = []
        for size in actions_size:
            temp_tensor = torch.empty(buffer_size, size, dtype=torch.float, device=device)
            self.actions.append(temp_tensor)
        # 每个智能体都有一个reward
        self.rewards = torch.empty(len(actions_size), buffer_size, dtype=torch.float, device=device)
    ...
    def add(self, transition):
        ...
        for key, value in actions.items():
            for i in range(value.shape[0]):
                self.actions[idx][self.count] = value[i]
                idx += 1
        for idx in range(rewards.shape[0]):
            self.rewards[idx][self.count] = rewards[idx]
        self.next_state[self.count] = next_state
    ...
    def sample(self, batch_size):
        ...
        sample_actions = [action[sample_idxs] for idx, action in enumerate(self.actions)]
        sample_rewards = [reward[sample_idxs] for idx, reward in enumerate(self.rewards) ]
