...
def create_container_data():
    # 假设下载带宽是1000 Mb/s
    downloading_bandwith = 1

    container_df = pd.DataFrame({
        'container_id': range(container_number),
        'container_size': np.random.randint(7, 200, container_number) # 单位是GB
    })
...
def create_edge_server_data():
    ...
    min_cpu_frequency = 8
    max_cpu_frequency = 8
    min_mem_capacity = 64000  # 单位是GB
    max_mem_capacity = 64000
    min_net_bandwith = 1000  # 单位是Mbits/s
    max_net_bandwith = 1000
    # min_read_bandwith = 100  # 单位是MB/s
    # max_read_bandwith = 100
    # min_write_bandwith = 100
    # max_write_bandwith = 100

    min_disk_storage = 1000  # 单位是GB,存储空间设置为1T
    max_disk_storage = 1000
...
def create_user_request_data():
    ...
    """
    total_time = 100
    min_cpu_frequency = 0.5
    max_cpu_frequency = 6
    min_mem_capacity = 200
    max_mem_capacity = 4500
    # min_read_bandwith = 0
    # max_read_bandwith = 5
    # min_write_bandwith = 0
    # max_write_bandwith = 10
    min_net_in_bandwith = 0
    max_net_in_bandwith = 100
    min_net_out_bandwith = 0
    max_net_out_bandwith = 700
    ...
    for i in range(user_number):
        user_col = [f"user_{i}_image", f"user_{i}_cpu", f"user_{i}_mem", f"user_{i}_net-in", f"user_{i}_net-out", f"user_{i}_lat"]
        # f"user_{i}_read", f"user_{i}_write"]
    ...
    for i in range(user_number * resource_category):
        ...
        # elif i % 7 == 5:  # read
        #     pre_data[:, i] = np.random.uniform(min_read_bandwith, max_read_bandwith, total_time)
        # elif i % 7 == 6:  # write
        #     pre_data[:, i] = np.random.uniform(min_write_bandwith, max_write_bandwith, total_time)
