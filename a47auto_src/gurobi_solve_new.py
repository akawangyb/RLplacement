...
for t in time_slots:
    ...
    for r_id, r in enumerate(slot_requests[t]):
        ...
        for k in x[req_id]:
            for s_id, s in enumerate(servers):
                current_allocation[r_id][s_id] = (x[req_id][k].X == 1)
            current_allocation[r_id][server_num] = (x[req_id]['cloud'].X == 1)
...
def save_to_yaml(deploy, alloc, filename):
    ...
    # 转换数据结构为适合 YAML 的格式
    yaml_data = {}
    yaml_data['deployment'] = deploy
    yaml_data['allocation'] = alloc
    # for t in time_slots:
        # yaml_data['deployment'][t-1]=deploy[t]
        # yaml_data['allocation'][t-1]=alloc[t]
    # # 处理部署结果
    # for t in sorted(deploy.keys()):
    #     time_key = str(t)
    #     yaml_data['deployment'][time_key] = OrderedDict()
    #     for server, containers in deploy[t].items():
    #         yaml_data['deployment'][time_key][str(server)] = containers
    #
    # # 处理分配结果
    # for t in sorted(alloc.keys()):
    #     time_key = str(t)
    #     yaml_data['allocation'][time_key] = OrderedDict()
    #
    #     # 处理边缘分配
    #     yaml_data['allocation'][time_key]['edge'] = OrderedDict()
    #     for server, req_ids in alloc[t]['edge'].items():
    #         yaml_data['allocation'][time_key]['edge'][str(server)] = req_ids
    #
    #     # 处理云分配
    #     yaml_data['allocation'][time_key]['cloud'] = alloc[t]['cloud']
