...
if __name__ == "__main__":
    ...
    for episode in range(config.EPOCHS):
        ...
        while not done:
            # 选择动作
            action, load_logp, value = agent.select_action(state)
            assert action.shape == (env.servers_number,), \
                f"not shape {action.shape}, env.servers_number {env.servers_number}"
            # 模拟环境返回奖励和新状态
            next_state, reward, done, info = env.step(action)
            # 存储经验
            agent.store_transition(state=state, action=action, reward=reward,
