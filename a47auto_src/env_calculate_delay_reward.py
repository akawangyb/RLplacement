...
class EdgeDeploymentEnv:
    ...
    def step(self, action: Tuple) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        for r_id, r in enumerate(self.current_requests):
            target = request_assign[r_id]
            # 计算时延
            if target == self.servers_number:
                compute_reward -= r['compute_delay']
                prop_reward -= self.L_c
            else:
                # 边缘计算时延（含干扰）
                server_name = self.servers[target]['server_id']
                compute_reward -= r['compute_delay'] * (1 + self._get_interference_coeff(server_name))
