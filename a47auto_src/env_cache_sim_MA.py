...
class EdgeDeploymentEnv:
    ...
    def compute_constraint_violation(self, action: np.ndarray):
        ...
        for server_id, ele in enumerate(new_mem_used):
            max_mem = self.partial * self.servers[server_id]['mem_capacity']
            violation[server_id] = max(0, ele * 1.0 / max_mem - 1)
            # violation[server_id] = ele * 1.0 / max_mem
        # return violation
        # print('new_mem_used', new_mem_used)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        for server_idx, server in enumerate(self.servers):
            ...
            for c_idx in range(self.containers_number):
                if new_loaded[c_idx] - prev_loaded[c_idx] == 1:
                    # 计算镜像加载时延
        ...
        for n in range(self.servers_number):
            ...
            for c in range(self.containers_number):
                if self.server_states[server_name]['loaded_containers'][c] == 1:
                    if d_m + self.h_c_map[self.containers[c]] > \
                            self.partial * self.servers[n]['mem_capacity']:
                        invalid_load[n] += 1
                    else:
                        d_m += self.h_c_map[self.containers[c]]
                # d_m += self.server_states[server_name]['loaded_containers'][c] * \
                #        self.h_c_map[self.containers[c]]
                # if d_m > self.partial * self.servers[n]['mem_capacity']:
        ...
        # 总奖励
        # reward = hits - container_reward - self.penalty * invalid_load
        for n in range(self.servers_number):
