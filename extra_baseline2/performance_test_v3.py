...
def eval_model_solution(agent, env, is_valid=True):
    """
    测试用深度模型的性能
    :param agent:
    :return:
...
# def eval_a2c(agent, env, nums):
#     final_total_reward = 0
#     final_episode_reward = [0]*24
#     final_episode_interference = [0]*24
#     final_episode_time = [0]*24
#     # for i in range(nums):
#         state, done = env.reset()
#         total_reward = torch.zeros(env.container_number)
#         episode_reward = []
...
#
#         total_reward = torch.sum(total_reward).item()
#         # final_total_reward += total_reward
#         # for i in range(len(episode_reward)):
#         #     print(i)
#         #     final_episode_reward[i] += episode_reward[i]
#         #     final_episode_interference[i] += episode_interference[i]
#         #     final_episode_time[i] += episode_time[i]
#
#     return final_total_reward, final_episode_reward, final_episode_interference, final_episode_time
...
def OLAIA(env: CustomEnv, trade_factor=0.6):
    """
    :param env:
    """
    total_reward = torch.zeros(env.container_number)
...
def TeLa(env: CustomEnv, trade_factor=0.0):
    """
...
if __name__ == '__main__':
    ...
    ddpg, bc_ddpg, td3, bc, a2c = _
    # a2c
    a2c_agent = A2C(state_dim=state_dim,
    ...
    print(a2c)
    a2c_agent.actor.load_state_dict(torch.load(a2c))
    a2c = eval_model_solution(a2c_agent, env=env)
    # mean = 0
    # for i in range(10):
    #     mean += eval_model_solution(a2c_agent, env=env)[0]
    # print(mean)
    # print(a2c[0])
    # a2c[0] = mean
    print(a2c[0])
    ...
    # td3 = eval_model_solution(agent, env=env)
    # print('td3 complete')
    # # ddpg
    # agent.actor.load_state_dict(torch.load(ddpg))
    # ddpg = eval_model_solution(agent, env=env)
    # print('ddpg complete')
    #
    # # # bc
    # agent.actor.load_state_dict(torch.load(bc))
    # bc = eval_model_solution(agent, env=env)
    # print('bc complete')
    #
    # #
    # # bc-ddpg
    ...
    # #
    #
    # OLAIA = OLAIA(env, trade_factor=0.8)
    # print(OLAIA[0])
    #
    # tela = TeLa(env, )
    # print(tela[0])
