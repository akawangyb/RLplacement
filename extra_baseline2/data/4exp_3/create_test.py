...
container_number = createData.container_number
...
def create_user_request_data():
    ...
    for i in range(user_number * resource_category):
        ...
        if i % resource_category == 0:  # cpu 列
            pre_data[:, i] = generate_data(tag, min_cpu_frequency, max_cpu_frequency) * 0.8
        elif i % resource_category == 1:  # mem 列
            pre_data[:, i] = generate_data(tag, min_mem_capacity, max_mem_capacity) * 0.8
        elif i % resource_category == 2:  # net-in
            pre_data[:, i] = generate_data(tag, min_net_in_bandwith, max_net_in_bandwith) * 0.8
        elif i % resource_category == 3:  # net-out
            pre_data[:, i] = generate_data(tag, min_net_out_bandwith, max_net_out_bandwith) * 0.8
