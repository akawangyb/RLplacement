...
def draw_interference_fig(ax, exp_name, title, top=-1, edge_server_number=0):
    ...
    ax.legend(prop=legend_font, loc='upper right', ncol=3)
...
def draw_delay_fig(ax, exp_name, title, top=-1, edge_server_number=0):
    ...
    for i in range(number):
        ...
        ax.plot(range(24), [sum(ele) / (1e2 * edge_server_number) for ele in values[i]],
                label=labels[i], linewidth=3.0, markersize=10,
                color=color[i], marker=marker[i])
    ax.set_xlim(0, 23)
    if top != -1:
    ...
    plt.xticks(range(24), fontproperties=roman_font_prop, size=20)
    plt.yticks(fontproperties=roman_font_prop, size=20)
    ax.annotate(r'$\times 10^2$', xy=(0, 1.01), xycoords='axes fraction', fontproperties=roman_font_prop)
    ax.set_xlabel('时隙', fontproperties=kai_font_prop)
...
exp1_title_list = [
    '$(\mathrm{a})$ 容器数量设置为$30$的每时隙干扰因子对比',
    '$(\mathrm{b})$ 容器数量设置为$50$的每时隙干扰因子对比',
    '$(\mathrm{c})$ 容器数量设置为$70$的每时隙干扰因子对比',
    '$(\mathrm{d})$ 容器数量设置为$30$的每时隙平均时延对比',
    '$(\mathrm{e})$ 容器数量设置为$50$的每时隙平均时延对比',
    '$(\mathrm{f})$ 容器数量设置为$70$的每时隙平均时延对比',

]
exp2_title_list = [
    '$(\mathrm{a})$ 打车服务分布采样的每时隙干扰因子对比',
    '$(\mathrm{b})$ 通信服务分布采样的每时隙干扰因子对比',
    '$(\mathrm{c})$ 镜像服务分布采样的每时隙干扰因子对比',
    '$(\mathrm{d})$ 打车服务分布采样的每时隙平均时延对比',
    '$(\mathrm{e})$ 通信服务分布采样的每时隙平均时延对比',
    '$(\mathrm{f})$ 镜像服务分布采样的每时隙平均时延对比',
]
...
# exp1_para = [0.25, 0.35, 2.1, 3.1, 5.4, 16]
exp1_para = [0.43, 0.29, 0.29, 7.1, 6.5, 6.7]
exp2_para = [0.35, 0.22, 0.20, 13.0, 10.0, 9.5]
exp3_para = [0.49, 0.57, 0.48, 9.2, 10.5, 8.5]
edge_service_number = [40, 40, 40]
para = exp2_para
# para = std_para
title_list = exp2_title_list
# exp_list = ['4exp_1', '4exp_2', '4exp_3', '4exp_1', '4exp_2', '4exp_3']
exp_list = ['5exp_1', '5exp_2', '5exp_3', '5exp_1', '5exp_2', '5exp_3']
# exp_list = ['6exp_1', '6exp_2', '6exp_3', '6exp_1', '6exp_2', '6exp_3']

index = 0
ax1 = fig.add_subplot(gs[0, 0])
draw_interference_fig(ax1, exp_list[index], title_list[index], para[index], edge_service_number[0])
index += 1

ax2 = fig.add_subplot(gs[1, 0])
draw_interference_fig(ax2, exp_list[index], title_list[index], para[index], edge_service_number[1])
index += 1

ax3 = fig.add_subplot(gs[2, 0])
draw_interference_fig(ax3, exp_list[index], title_list[index], para[index], edge_service_number[2])
index += 1

ax4 = fig.add_subplot(gs[0, 1])
draw_delay_fig(ax4, exp_list[index], title_list[index], para[index], edge_service_number[0])
index += 1

ax5 = fig.add_subplot(gs[1, 1])
draw_delay_fig(ax5, exp_list[index], title_list[index], para[index], edge_service_number[1])
index += 1

ax6 = fig.add_subplot(gs[2, 1])
draw_delay_fig(ax6, exp_list[index], title_list[index], para[index], edge_service_number[2])
index += 1

plt.subplots_adjust(top=0.98, bottom=0.08, left=0.070, right=0.99)
plt.show()
