...
# path=r'../log_res/ablation2/4-1/actor/experiment_epochs_env-20250228-132012/output_info.log'
path = [r'../log_res/ablation2/4-1/actor/experiment_epochs_env-20250228-132012/output_info.log',
        r'../log_res/ablation2/4-1/actor']
...
draw_imitation_fig1(ax1, title[0], path[0],annotate_text=r'$\times 10^6$', critic=False)

ax2 = fig.add_subplot(gs[2:, 0])
file_paths = find_name_log_folders(path[1], agent_name='imitation_learning')
draw_imitation_fig2(ax2, file_paths, title=title[1],annotate_text=r'$\times 10^6$', tag='epochs')

index = 1
# 右边上面的第一个图
ax3 = fig.add_subplot(gs[0, 1])
path = r'../log_res/ablation2/4-1/actor/imitation_learning_env-033-20250302-232709/output_info.log'
draw_sub_fig(ax3, title='$\mathrm{(c)}$ $\mathrm{Step=33}$的后续训练收益',annotate_text=r'$\times 10^5$', file_path=path, i=index)
index += 1
# 右边上面的第二个图
ax4 = fig.add_subplot(gs[1, 1])
path = r'../log_res/ablation2/4-1/actor/imitation_learning_env-060-20250302-233915/output_info.log'
draw_sub_fig(ax4, title='$\mathrm{(d)}$ $\mathrm{Step=60}$的后续训练收益',annotate_text=r'$\times 10^5$', file_path=path, i=index)
index += 1
# 右边下面的第一个图
ax5 = fig.add_subplot(gs[2, 1])
path = r'../log_res/ablation2/4-1/actor/imitation_learning_env-098-20250302-215340/output_info.log'
draw_sub_fig(ax5, title='$\mathrm{(e)}$ $\mathrm{Step=98}$的后续训练收益',annotate_text=r'$\times 10^5$', file_path=path, i=index)
index += 1
# 右边下面的第二个图
ax6 = fig.add_subplot(gs[3, 1])
path = r'../log_res/ablation2/4-1/actor/imitation_learning_env-140-20250302-224417/output_info.log'
draw_sub_fig(ax6, title='$\mathrm{(f)}$ $\mathrm{Step=140}$的后续训练收益',annotate_text=r'$\times 10^5$', file_path=path, i=index)
