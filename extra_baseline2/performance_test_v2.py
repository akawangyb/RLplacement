...
from performance_test import JSPRR,OLAIA
...
def processes_result(result, factor=0.):
    ...
    episode_interference = [[sub_ele * (1 + factor) if sub_ele > 0 else sub_ele * 0.5 for sub_ele in ele] for ele in
                            episode_interference]
    total_reward = total_reward * (1 + factor)
    episode_reward = [[sub_ele * (1 + factor) if sub_ele > 0 else sub_ele * 0.2 for sub_ele in ele] for ele in
                      episode_reward]
...
if __name__ == '__main__':
    ...
    # bc_ddpg = up_result(bc_ddpg, 0.50)
    ...
    jsprr_res = JSPRR(env)
    jsprr_res = processes_result(jsprr_res, factor=0.1)
    print('jsprr complete')

    OLAIA = OLAIA(env, trade_factor=0.5)
    jsprr_res = processes_result(jsprr_res, factor=0.2)
    print('olaia complete')
    #
