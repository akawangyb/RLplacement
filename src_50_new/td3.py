...
class ActorNet(torch.nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_dim):
        ...
        #         gain = init.calculate_gain('relu')  # 计算ReLU的增益
...
class TD3:
    ...
    def update(self, batch, actor_update: bool = True):
        self.actor.train()
        ...
        if actor_update:
            ...
            actor_loss = -torch.mean(
                torch.min(
                    ...
                    self.critic_2(state, self.actor(state))
                )
            )
            actor_loss.backward()
    ...
    def learn(self, transition_dict, epoch=10, learning_rate=0.001):
