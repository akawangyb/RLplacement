...
class EdgeDeploymentEnv:
    def __init__(self, dataset_dir):
        ...
        self.server_states = {server['server_id']: {
            'loaded_containers': [0] * self.containers_number,
            'cpu_used': 0,
            'mem_used': 0,
            'upload_used': 0,
            'download_used': 0,
    ...
    def mask_load_action(self, load_action):
        # 返回一个过滤后的容器
        container_reward = 0.0
        load_invalid_actions = 0
        container_deploy = load_action
        assert container_deploy.shape == (
            self.servers_number, self.containers_number), f'container deploy shape error {container_deploy.shape}'
        for server_idx, server in enumerate(self.servers):
            mem = 0
            for container_id, contaner in enumerate(self.containers):
                mem += container_deploy[server_idx][container_id] * self.h_c_map[contaner]
                if mem > self.servers[server_idx]['mem_capacity']:
                    load_invalid_actions += 1
                    container_deploy[server_idx][container_id] = 0

        return container_deploy
    ...
    def step(self, action: Tuple) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        for req_idx, req in enumerate(self.current_requests):
            target = int(request_assign[req_idx])
            # target表示目标服务器
            # 检查目标合法性
        ...
        reward = container_reward + compute_reward + prop_reward - self.penalty * (
                    invalid_actions + load_invalid_actions)
        ...
        info = {
            ...
            'load_invalid_actions': load_invalid_actions
        }
    ...
    def get_alloc_mask(self):
        """
        输入：当前环境状态（包含服务器资源使用情况）
        输出：形状为 [MAX_REQUESTS, N_SERVERS+1] 的二进制掩码矩阵
        就是根据容器的部署动作，来筛选请求分配动作
        """
        return None
        mask = np.ones((self.max_requests, self.servers_number + 1), dtype=np.float32)
        for req_idx in range(self.max_requests):
            if req_idx >= len(self.current_requests):
                break
            req_cpu = self.current_requests[req_idx]['cpu_demand']
            req_mem = self.current_requests[req_idx]['mem_demand']
            req_upload = self.current_requests[req_idx]['upload_demand']
            req_download = self.current_requests[req_idx]['download_demand']
            for server_idx in range(self.servers_number):
                # 计算服务器剩余资源
                remain_cpu = self.servers[server_idx]['cpu_capacity']
                remain_mem = self.servers[server_idx]['mem_capacity']
                remain_upload = self.servers[server_idx]['upload_capacity']
                remain_download = self.servers[server_idx]['download_capacity']
                if (req_cpu < remain_cpu) or (req_mem < remain_mem) or (req_upload < remain_upload) or (
                        req_download < remain_download):
                    mask[req_idx, server_idx] = 0
            mask[req_idx, -1] = 1
        return mask
    ...
    def get_loading_mask(self):
        ...
        """
        输入：当前环境状态（包含容器加载状态）
        输出：形状为 [N_SERVERS, N_CONTAINERS] 的二进制掩码矩阵
        """
        mask = np.zeros((self.servers_number, self.containers_number), dtype=np.float32)
        for srv_idx in range(self.servers_number):
            for ctn_idx in range(self.containers_number):
                continue
                # 仅允许加载未驻留的容器
                # if self.servers[srv_idx].containers[ctn_idx] == 0:
                #     mask[srv_idx, ctn_idx] = 1
        return mask
