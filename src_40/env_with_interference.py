...
class CustomEnv(gym.Env):
    ...
    def model_solve(self, solution_cnt=1):
        """
        :return: 不松驰直接求解
        """
        ts = self.timestamp
        ...
        m.setParam('PoolSolutions', solution_cnt)
        ...
        for i in range(m.SolCount):
            x_result.append(np.array([[x[i, j].x for j in x_columns] for i in x_rows]))
        return x_result

    def model_solve_max_edge(self):
        ts = self.timestamp
        # 创建模型
        m = gp.Model("example")
        m.setParam('OutputFlag', 0)
