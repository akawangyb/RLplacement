...
from customENV.gpu_env_v1 import CustomEnv
...
for i_episode in range(config.num_episodes):
    ...
    while not done:
        ...
        placing_actions = np.array(actions[:env.placing_agents_number])
        routing_actions = np.array(actions[-env.routing_agents_number:])
    ...
    if (i_episode + 1) % 10 == 0:
        ...
        ep_returns = evaluate(env, maddpg)
        # 保存最优的模型
