...
draw_sub_fig(ax4, title='$\mathrm{(d)Step=36}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边下面的第一个图
ax5 = fig.add_subplot(gs[2, 1])
path = r'../log_res/ablation/multi_exp_actor/imitation_learning_env-20250222-130542/output_info.log'
draw_sub_fig(ax5, title='$\mathrm{(e)Step=47}$的后续训练收益', file_path=path, i=index)
index += 1
# 右边下面的第二个图
ax6 = fig.add_subplot(gs[3, 1])
path = r'../log_res/ablation/multi_exp_actor/imitation_learning_env-20250222-132615/output_info.log'
draw_sub_fig(ax6, title='$\mathrm{(f)Step=60}$的后续训练收益', file_path=path, i=index)
