...
def base_opt(env_name, data_dir):
    ...
    # 创建实验单独的文件夹
    father_log_directory = os.path.join(father_log_directory, data_dir)
    if not os.path.exists(father_log_directory):
        os.makedirs(father_log_directory)
    ...
    # 拷贝文件
    shutil.copy('train_config.yaml', os.path.join(log_dir, 'train_config.yaml'))
...
def find_solution_file(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        ...
        for folder in dirs:
            ...
            elif folder.startswith('a2c'):
                a2c = os.path.join(root, folder)
    ...
    a2c = os.path.join(a2c, 'a2c_actor.pth')
    # return ddpg, bc_ddpg, td3, bc ,a2c
    return ddpg, bc_ddpg, td3, bc ,a2c
