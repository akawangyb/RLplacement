...
class ReplayBuffer:
    ...
    def add(self, state, load_action, reward, next_state, done, old_load_logp,cost):
        """ 存储单步经验 """
        self.buffer.append((
    ...
    def sample(self, batch_size):
        ...
        # 解包为张量
        states, load_acts, rewards, next_states, dones, old_load_logps,cost = zip(*batch)
        return (
...
class PPO:
    ...
    def update(self):
        ...
        for _ in range(EPOCHS):
            # 采样批次数据
            states, load_acts, rewards, next_states, dones, old_load_logps, constraints = \
            ...
            assert load_acts.shape == (BATCH_SIZE, env.servers_number, env.containers_number), \
                f"shape not {states.shape}"
            # 计算约束违反量
            avg_constraint = constraints.mean()
            constraint_violation = avg_constraint - C_MAX
            ...
            policy_loss = -torch.min(surr1, surr2).mean() - self.lambda_param * constraint_violation
            ...
            self.lambda_param.grad = -self.lambda_param.grad  # 梯度取反
            self.lambda_optim.step()
            self.lambda_param.data = torch.clamp(self.lambda_param.data, min=0)  # 乘数非负
...
if __name__ == "__main__":
    ...
    for episode in range(1000):
        ...
        while not done:
            ...
            mask_load_act = env.mask_load_action(load_act)
            ...
            invalid_load_actions += info['load_invalid_actions']
            # 存储经验
            print(load_act)
            agent.buffer.add(state, load_act, -ppo_reward, next_state, done, load_logp, constraint_cost)
        ...
        print("invalid actions", invalid_actions, "invalid load actions", invalid_load_actions)
