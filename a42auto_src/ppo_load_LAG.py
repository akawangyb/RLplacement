...
class PPO:
    def __init__(self):
        ...
        # 初始化拉格朗日乘子（每个约束对应一个）
        self.lag_multiplier = nn.Parameter(torch.tensor(0.1), requires_grad=True)
        self.multiplier_lr = 1e-4  # 乘子学习率
    ...
    def update(self):
        ...
        for _ in range(EPOCHS):
            # 采样批次数据
            states, load_acts, rewards, next_states, dones, old_load_logps, constraint_violation = \
            ...
            # 这个值由环境反馈，在记忆中存储
            # print(actor_loss)
            constraint_violation = torch.mean(constraint_violation)
            # assert constraint_violation.shape == actor_loss, \
            #     f"actor loss shape {actor_loss.shape},\
            #     but constraint violation shape {constraint_violation.shape}"
            # 拉格朗日增强的Actor损失
            augmented_actor_loss = actor_loss + self.lag_multiplier * constraint_violation

            # 总损失（Critic和熵项不变）
            total_loss = augmented_actor_loss + critic_loss + entropy_loss
            ...
            (-self.lag_multiplier * constraint_violation.detach()).backward()  # 最大化乘子项
            self.multiplier_optim.step()
            self.lag_multiplier.data.clamp_(min=0.0)  # 乘子非负
...
if __name__ == "__main__":
    ...
    for episode in range(1000):
        ...
        while not done:
            ...
            # 存储经验
            agent.buffer.add(state, load_act, reward, next_state, done, load_logp, constraint_violations)
            total_reward += reward
        agent.update()
