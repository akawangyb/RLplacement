...
def gurobi_solve_relax(y: np.ndarray):
    """
    :param y: 容器的加载动作
    ...
    for s in servers:
        ...
        mem_usage = gp.quicksum(
            ...
            y[c, server_id] * df_requests[df_requests['service_type'] == c]['image_size'].mean() * 1024  # MB
            for c in containers
        )
    ...
    for r in requests:
        ...
        for s in servers:
            server_id = s['server_id']
            model.addConstr(
                x[req_id][server_id] <= y[server_id][c],
...
def validate_action(t, x_rounded, servers, requests_t, y):
    """校验并修复当前时隙解"""
    # 校验请求分配唯一性
