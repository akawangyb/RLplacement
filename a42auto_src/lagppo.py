...
class PPO:
    ...
    def update(self):
        ...
        for _ in range(5):
            ...
            # 计算平均约束违反量
            avg_cost = costs.mean()
            constraint_violation = torch.relu(avg_cost - C_MAX + 1e-3)  # COST_MAX为约束阈值
            ...
            # 拉格朗日惩罚项
            policy_loss = -torch.min(surr1, surr2).mean() + self.lag_multiplier * constraint_violation
            # ------------ 价值损失计算 ------------
            ...
            # 更新拉格朗日乘子
            lambda_loss = -self.lag_multiplier * constraint_violation
